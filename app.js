...
const addRow = (d, x) => {
  ...
  svg.append("text").datum(d.level).attr('class', 'row-title').attr('transform', `translate(0,${y(d.level + 1) * 3 - 10})`).text(titleText);

  let row = svg.append('g').datum(d.level).attr('class', 'row');

  row.attr('opacity', 0).attr('transform', `translate(0,${y(d.level) || 0})`).transition().duration(777).attr('opacity', 1).attr('transform', `translate(0,${y(d.level + 1) * 3})`);
