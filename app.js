...
const colours = [['#add8e6', '#a1c2dd', '#96acd4', '#8997cb', '#7c82c2', '#6f6db9', '#6059b0', '#5144a7', '#3f309e', '#2a1b94', '#00008b'], // Asset class level
...
const addRow = (d, x) => {
  ...
  svg.selectAll('.row-title').filter(dd => dd >= d.level).transition().attr('opacity', 0).remove();

  // Append row title text
  svg.append("text").datum(d.level).attr('class', 'row-title').attr('transform', `translate(0,${y(d.level + 1) * 2 - 10})`).text(titleText);

  // Create and append row
  let row = svg.append('g').datum(d.level).attr('class', 'row');

  // Transition animation attributes
  row.attr('opacity', 0).attr('transform', `translate(0,${y(d.level) || 0})`).transition().duration(777).attr('opacity', 1).attr('transform', `translate(0,${y(d.level + 1) * 2})`);
  ...
  let legend = svg.append('g').data(d.children).attr('class', 'legend');

  legend.attr('transform', `translate(0,${y(d.level) || 0})`).transition().duration(777).attr('opacity', 1).attr('transform', `translate(0,${y(d.level + 1) * 3})`);

  let box = legend.selectAll('box').data(d.children).enter().append('box').attr('fill', dd => dd.color).attr('x', x(d.x0 || 0)).attr('width', '20').attr('y', '20').attr('height', '20');
};
